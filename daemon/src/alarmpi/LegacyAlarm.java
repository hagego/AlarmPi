 ...
 */
public class LegacyAlarm implements Serializable {
	 ...
	 */
	static List<LegacyAlarm> readAll() {
		List<LegacyAlarm> alarmList = new LinkedList<LegacyAlarm>();
		
		try {
			int count=0;
			Preferences prefs = Preferences.userNodeForPackage(LegacyAlarm.class);
			for(String id:prefs.keys()) {
				byte bytes[] = prefs.getByteArray(id, null);
				if(bytes!=null) {
					ObjectInputStream ois = new ObjectInputStream(new ByteArrayInputStream(bytes));
					LegacyAlarm alarm = (LegacyAlarm)ois.readObject();
					...
					if(alarm.id>=LegacyAlarm.nextId) {
						LegacyAlarm.nextId = alarm.id+1;
					}
				}
				count++;
			}
	...
	// private members
	private static final Logger log              = Logger.getLogger( LegacyAlarm.class.getName() );
